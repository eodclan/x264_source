...
check_access(UC_DEV);
security_tactics();
x264_bootstrap_header("Menu Management");
...
if ($ac == "reorder") 
{ 
  foreach($or as $id => $position) mysql_query("UPDATE `staffacpmenu` SET `order`='$position' WHERE id='$id'"); header("Refresh: 0; url=".$_SERVER["PHP_SELF"].""); 
}
...
print"
              ...
              <table id='example1' class='table table-bordered table-striped'>
                ...
                <tr>
                  <td>";  
  $res = mysql_query("SELECT * FROM `staffacpmenu` WHERE `id`='$gid' LIMIT 1");
  while ($arr = mysql_fetch_array($res, MYSQL_BOTH)) 
  { 
    $arr["name"] = $arr["name"]; $arr["url"] = $arr["url"];
    if ($arr["type"] == "unter") 
    { 
      ?>
<!-- action=editunter !!BEGINNT!! -->        
      ...
      <?php 
      $res2 = mysql_query("SELECT `id`, `name` FROM `staffacpmenu` WHERE `type`='haupt' ORDER BY `order` ASC"); 
      while ($arr2 = mysql_fetch_array($res2, MYSQL_BOTH)) 
      { 
        $selected = ($arr2["id"] == $arr["haupt"]) ? " selected=\"selected\"" : ""; 
        ?>
                            <option value="<?=$arr2["id"]?>" <?=$selected?>><?=$arr2["name"]?></option>
        <?php 
      } 
      ?>
                          </select>
...
if ($ac == "editunter" && $id != NULL && $na != NULL  && $ur != NULL && $cl != NULL && $ha != NULL) 
{   
  mysql_query("UPDATE `staffacpmenu` SET `name`='$na', `url`='$ur', `class`='$cl', `haupt`='$ha' WHERE id='$id'"); 
  header("Refresh: 0; url=".$_SERVER["PHP_SELF"].""); 
}
if ($ac == "editsect"  && $id != NULL && $ti != NULL && $ur != NULL && $cl != NULL) 
{   
  mysql_query("UPDATE `staffacpmenu` SET `name`='$ti', `url`='$ur', `class`='$cl', `haupt`='0' WHERE id='$id'"); 
  header("Refresh: 0; url=".$_SERVER["PHP_SELF"].""); 
}
...
{ 
  if ($co == "yes") 
  { 
    mysql_query("DELETE FROM `staffacpmenu` WHERE `id`='$gid' LIMIT 1"); 
    header("Refresh: 0; url=".$_SERVER["PHP_SELF"]."");  
  }
  else 
  {
print"
                    <div class='row'>
                        <div class='col-lg-12'>
                            <div class='card'>
                                <div class='card-header'>
                                    <i class='fa fa-edit'></i>Menu Punkt L&ouml;schen
                                    <div class='card-actions'>
                                        <a href='#' class='btn-close'><i class='icon-close'></i></a>
                                    </div>
                                </div>
...
print"
                    <div class='row'>
                        <div class='col-lg-12'>
                            <div class='card'>
                                <div class='card-header'>
                                    <i class='fa fa-edit'></i>Menu Punkt Hinzuf&uuml;gen
                                    <div class='card-actions'>
                                        <a href='#' class='btn-close'><i class='icon-close'></i></a>
                                    </div>
                                </div>
  ...
  <?php 
  $res = mysql_query("SELECT `id`, `name` FROM `staffacpmenu` WHERE `type`='haupt' ORDER BY `order` ASC");
  while ($arr = mysql_fetch_array($res, MYSQL_BOTH)) 
  { 
...
if ($ac == "addnewunter" && $na != NULL && $ur != NULL && $cl != NULL && $ha != NULL) 
{ 
  $res = mysql_query("SELECT MAX(`order`) FROM `staffacpmenu` WHERE `type`='unter' AND `haupt`='$ha'"); 
  while ($arr = mysql_fetch_array($res, MYSQL_BOTH)) 
  $order = $arr[0] + 1; 
  mysql_query("INSERT INTO `staffacpmenu` (`type`, `name`, `url`, `class`, `haupt`, `order`) VALUES ('unter', '$na', '$ur', '$cl', '$ha', '$order')"); 
  header("Refresh: 0; url=".$_SERVER["PHP_SELF"]."");  
}
if ($ac == "addnewsect" && $ti != NULL && $ur != NULL && $cl != NULL) 
{   
  $res = mysql_query("SELECT MAX(`order`) FROM `staffacpmenu` WHERE `type`='haupt'"); 
  while ($arr = mysql_fetch_array($res, MYSQL_BOTH)) 
  $order = $arr[0] + 1; 
  mysql_query("INSERT INTO `staffacpmenu` (`type`, `name`, `url`, `class`, `haupt`, `order`) VALUES ('haupt', '$ti', '$ur', '$cl', '0', '$order')"); 
  header("Refresh: 0; url=".$_SERVER["PHP_SELF"].""); 
}
...
print"
                    <div class='row'>
                        <div class='col-lg-12'>
                            <div class='card'>
                                <div class='card-header'>
                                    <i class='fa fa-edit'></i>Backend ACP Menu Management
                                    <div class='card-actions'>
                                        <a href='#' class='btn-close'><i class='icon-close'></i></a>
                                    </div>
                                </div>
              ...
              <table id='example1' class='table table-bordered table-striped'>
                ...
                <tr>
                  <td>";
$res = mysql_query("SELECT `id`, `name`, `class`, `order` FROM `staffacpmenu` WHERE `type`='haupt' ORDER BY `order` ASC"); 
while ($arr = mysql_fetch_array($res, MYSQL_BOTH)) 
{ 
...
$res = mysql_query("SELECT `id`, `name`, `class`, `haupt`, `order` FROM `staffacpmenu` WHERE `type`='unter' ORDER BY `order` ASC"); 
while ($arr = mysql_fetch_array($res, MYSQL_BOTH)) 
{ 

...
class IMDB {
    ...
    public function __construct($strSearch, $intCache = 1440) {
        ...
        if (is_writable($this->_strRoot . '/imdb_posters/') || mkdir($this->_strRoot . '/imdb_posters/')) {
            $this->_bolPoster = true;
        }
        else {
            throw new IMDBException($this->_strRoot . '/imdb_posters/ is not writable!');
        }
        if (is_writable($this->_strRoot . '/imdb_cache/') || mkdir($this->_strRoot . '/imdb_cache/')) {
            $this->_bolCache = true;
        }
        else {
            throw new IMDBException($this->_strRoot . '/imdb_cache/ is not writable!');
        }
        // cURL.
    ...
    private function fetchUrl($strSearch) {
        ...
        // Otherwise try to find one.
            ...
            if ($fRedirect = @file_get_contents($this->_strRoot . '/imdb_cache/' . md5($this->_strUrl) . '.redir')) {
        ...
        $fCache = $this->_strRoot . '/imdb_cache/' . md5($this->_strId) . '.imdb_cache';
        ...
        if ($bolUseCache) {
            ...
            $fRedirect = $this->_strRoot . '/imdb_cache/' . md5($this->_strUrl) . '.redir';
    ...
    private function saveImage($_strUrl) {
        ...
        if (preg_match('/imdb-share-logo.gif/', $_strUrl) && file_exists('imdb_posters/not-found.jpg')) {
            return 'imdb_posters/not-found.jpg';
        }

        $strFilename = $this->_strRoot . '/imdb_posters/' . $this->_strId . '.jpg';
        if (file_exists($strFilename)) {
            return 'imdb_posters/' . $this->_strId . '.jpg';
        }
    ...
    public function getCountryAsUrl($strTarget = '') {
        if ($this->isReady) {
            ...
            if (count($arrReturned[2])) {
                foreach ($arrReturned[2] as $i => $strName) {
                    $arrReturn[] = '<a href="http://www.imdb.com/country/' . trim($aMatch[1][$i]) . '/"' . ($sTarget ? ' target="' . $sTarget . '"' : '') . '></a>';
                }
                return implode($this->strSeperator, $arrReturn);
            }
        }
    ...
    public function getLocationAsUrl($strTarget = '') {
        if ($this->isReady) {
            if ($strReturn = $this->matchRegex($this->_strSource, IMDB::IMDB_LOCATION, 2)) {
                return '<a href="http://www.imdb.com/search/title?locations=' . IMDBHelper::cleanString($aMatch[1][$i]) . '"' . ($sTarget ? ' target="' . $sTarget . '"' : '') . '>see more</a>';
            }
        }
        return $this->strNotFound;
    }

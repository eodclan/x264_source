    ...
    foreach ($arr as $e) {
        ...
        if ($e['username']) {
            if (($e['tanonymous'] == 'yes' && $e['owner'] == $e['userid'] || $e['anonymous'] == 'yes' && $CURUSER['id'] != $e['userid']) && $CURUSER['class'] < UC_MODERATOR)
                ...
                $htmlout .= "<td><a href='userdetails.php?id=" . intval($e['userid']) . "'><b>" . htmlspecialchars($e['username']) . "</b></a></td>\n";
        } else
...
$res = mysql_query("SELECT * FROM torrents WHERE id = " . sqlesc($id)) or sqlerr(__FILE__, __LINE__);
...
echo x264_header("{$lang['peerslist_stdhead']}") . $HTMLOUT . x264_footer();
?>
